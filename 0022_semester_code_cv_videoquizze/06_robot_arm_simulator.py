...
class KinematicDataset(Dataset):
    ...
    def __init__(self, data_dir: str):
        ...
        print(f"Loaded dataset with {self.num_samples} samples from CSV")
...
def train_mode(args):
    """Run train mode with enhanced training features."""
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            for images, actions in val_loader:
                ...
                epoch_targets.extend(actions.cpu().numpy())
        ...
        if epoch % 5 == 0 or epoch == args.epochs - 1:
            ...
            print(f"  Best Val Accuracy: {best_val_accuracy:.2f}% | Patience: {patience_counter}/{patience_limit}")
    ...
    # Save model
    model_path = Path(args.output) / f"model_dof{args.dof}_epoch{epoch+1}.pth"
    ...
    plt.axhline(y=best_val_accuracy, color='r', linestyle='--', label=f'Best Val: {best_val_accuracy:.2f}%')
    plt.xlabel('Epoch', fontsize=12)
    ...
    if len(all_predictions) > 0:
        ...
        plt.title('Confusion Matrix (Best Epoch)', fontsize=14, fontweight='bold')
        plt.colorbar()
    ...
    if len(all_predictions) > 0:
        ...
        for action in range(action_space_size):
            mask = np.array(all_targets) == action
            if mask.sum() > 0:
                acc = (np.array(all_predictions)[mask] == action).mean() * 100
                per_action_accuracy.append(acc)
            else:
                per_action_accuracy.append(0)
        
        plt.bar(range(action_space_size), per_action_accuracy, color='steelblue')
        plt.xlabel('Action', fontsize=12)
        plt.ylabel('Accuracy (%)', fontsize=12)
        plt.title('Per-Action Accuracy', fontsize=14, fontweight='bold')
        plt.xticks(range(action_space_size))
    ...
    Best Val Accuracy: {best_val_accuracy:.2f}%
    ...
    plot_path = Path(args.output) / f"training_analysis_dof{args.dof}.png"
    plt.tight_layout()
...
def test_mode(args):
    """Run test mode with detailed performance evaluation."""
    if not TORCH_AVAILABLE:
        print("Error: PyTorch not available. Please install torch and related packages.")
        return 1
    ...
    try:
        ...
        if 'training_history' in checkpoint:
            ...
            print(f"\nTraining History:")
            print(f"  Best Validation Accuracy: {history.get('best_val_accuracy', 'N/A'):.2f}%")
            print(f"  Training Epochs: {len(history.get('train_losses', []))}")
    ...
    # Create neural network action generator with statistics
    def nn_action_generator(image):
        ...
        # Track statistics
        test_stats['total_steps'] += 1
        test_stats['distances_to_target'].append(chain.get_distance_to_target())
    ...
    # Create main window with test stats
    window = MainWindow(chain, 'test', nn_action_generator)
    window.test_stats = test_stats
    window.show()
    
    print(f"\nTest mode running. Neural network is controlling the arm.")
    print(f"Statistics will be tracked and displayed.")
    return app.exec()
